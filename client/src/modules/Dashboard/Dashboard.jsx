import { useState } from "react";
import User from "../../assets/user.svg";
import Input from "../../components/Input/Input";

const contactsStorage = [
    {
        id: 1,
        name: "Jonesy Doe",
        active: true,
        image: User,
    },
    {
        id: 2,
        name: "Jane Doe",
        active: false,
        image: User,
    },
    {
        id: 3,
        name: "Summer Doe",
        active: true,
        image: User,
    },
    {
        id: 4,
        name: "James Doe",
        active: false,
        image: User,
    },
    {
        id: 5,
        name: "Domino",
        active: false,
        image: User,
    },
    {
        id: 6,
        name: "Deadpool",
        active: false,
        image: User,
    },
    {
        id: 7,
        name: "Spiderman",
        active: true,
        image: User,
    },
    {
        id: 8,
        name: "James Doe",
        active: false,
        image: User,
    },
    {
        id: 9,
        name: "Domino",
        active: false,
        image: User,
    },
    {
        id: 10,
        name: "Deadpool",
        active: false,
        image: User,
    },
    {
        id: 11,
        name: "Spiderman",
        active: true,
        image: User,
    },
];

export default function Dashboard() {
    const [search, setSearch] = useState("");
    const [message, setMessage] = useState("");
    const [typing, setTyping] = useState(false);
    const [contacts, setContacts] = useState(contactsStorage);
    const [activeUser, setActiveUser] = useState({
        name: "Spiderman",
        active: true,
        image: User,
        typing: typing,
    });

    // Typing indicator logic
    const handleTyping = () => {
        setTyping(true);
        // Clear typing status after a delay
        setTimeout(() => setTyping(false), 3000); // Adjust delay as necessary
    };

    return (
        <div className="w-screen flex">
            <div className="w-1/4 h-screen bg-secondary overflow-scroll mx-4">
                <div className="flex justify-center items-center flex-1"></div>
                <Input
                    type="text"
                    placeholder="Search"
                    isRequired={false}
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    className="w-full border rounded h-10 p-2 my-2 focus:outline-primary"
                />
                <hr />
                <div>
                    <h1 className="text-xl text-left font-medium text-primary">
                        Messages
                    </h1>
                    <div className="overflow-scroll">
                        {contacts
                            .filter((contact) =>
                                contact.name
                                    .toLowerCase()
                                    .includes(search.toLowerCase())
                            )
                            .map(({ id, name, active, image }) => (
                                <div
                                    key={id}
                                    onClick={() => {
                                        setActiveUser({
                                            id: id,
                                            name: name,
                                            active: active,
                                            image: image,
                                        });
                                    }}
                                    className={`cursor-pointer ${
                                        id === activeUser.id
                                            ? "bg-white"
                                            : "bg-secondary"
                                    }`}
                                >
                                    <div className="flex justify-start items-center m-4">
                                        <div className="border-primary rounded-full">
                                            <img
                                                className="rounded-full"
                                                src={image}
                                                width={50}
                                                height={50}
                                            />
                                        </div>
                                        <div className="ml-2">
                                            <h3 className="text-base font-medium">
                                                {name}
                                            </h3>
                                            <p
                                                className={`text-xs font-normal italic ${
                                                    active
                                                        ? "text-green-500"
                                                        : "text-red-500"
                                                }`}
                                            >
                                                {active ? "Online" : "Offline"}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ))}
                    </div>
                </div>
            </div>
            <div className="w-3/4 h-screen bg-white flex flex-col items-left">
                <div className="flex flex-row items-center bg-secondary px-2">
                    <img
                        className="rounded-full"
                        src={activeUser.image}
                        width={50}
                        height={50}
                    />
                    <div className="m-4">
                        <p className="font-bold">
                            {activeUser.name}{" "}
                            <span
                                className={`text-xl font-bold italic ${
                                    activeUser.active
                                        ? "text-green-500"
                                        : "text-red-500"
                                }`}
                            >
                                {activeUser.active ? "•" : "•"}
                            </span>
                        </p>
                        <p className="text-xs font-normal italic text-gray-500">
                            {activeUser.typing ? "typing..." : ""}
                        </p>
                    </div>
                </div>
                <div className="h-full bg-white overflow-scroll">
                    <div className="px-10 py-14">
                        <div className="max-w-[40%] bg-secondary text-black rounded-b-xl rounded-tr-xl mr-auto p-4 my-2">
                            Hi Samaira!!!
                        </div>
                        <div className="max-w-[40%] bg-secondary text-black rounded-b-xl rounded-tr-xl mr-auto p-4 my-2">
                            Lorem ipsum dolor sit amet consectetur adipisicing
                            elit. Necessitatibus molestias nulla quibusdam,
                            cumque repellendus, assumenda at omnis iusto
                            obcaecati, tenetur quae repellat perferendis minus
                            placeat magnam recusandae culpa temporibus sed.
                        </div>
                        <div className="max-w-[60%] bg-primary-light text-white rounded-bl-xl rounded-t-xl ml-auto p-4 my-2">
                            Hey Spidey!
                        </div>
                        <div className="max-w-[60%] bg-primary-light text-white rounded-bl-xl rounded-t-xl ml-auto p-4 my-2">
                            Lorem ipsum dolor sit amet consectetur adipisicing
                            elit. Esse aperiam nesciunt quis cum, perspiciatis
                            vitae autem cumque illo et quos in libero
                            reiciendis, tempora, molestias nam illum sit quas
                            sed.
                        </div>
                        <div className="max-w-[40%] bg-secondary text-black rounded-b-xl rounded-tr-xl mr-auto p-4 my-2">
                            Lorem ipsum dolor sit amet consectetur adipisicing
                            elit. Necessitatibus molestias nulla quibusdam,
                            cumque repellendus, assumenda at omnis iusto
                            obcaecati, tenetur quae repellat perferendis minus
                            placeat magnam recusandae culpa temporibus sed.
                        </div>
                    </div>
                </div>
                <div className="px-2">
                    <hr />
                    <Input
                        type="text"
                        placeholder="Type your message..."
                        value={message}
                        onChange={(e) => {
                            setMessage(e.target.value);
                            handleTyping(); // Trigger typing indicator
                        }}
                        disabled={!activeUser.active}
                        className="w-full border rounded h-10 p-2 my-2 focus:outline-primary"
                    />
                </div>
            </div>
        </div>
    );
}
